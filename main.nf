...
workflow.onComplete {
    ...
    def subject = "[NF-hints] Successful: $workflow.runName"
    if(!workflow.success){
      subject = "[NF-hints] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    if (params.email) {
        try {
          ...
          log.info "[NF-hints] Sent summary e-mail to $params.email (sendmail)"
        } catch (all) {
          // Catch failures and try with plaintext
          [ 'mail', '-s', subject, params.email ].execute() << email_txt
          log.info "[NF-hints] Sent summary e-mail to $params.email (mail)"
        }
    }
    ...
    log.info "[NF-hints] Pipeline Complete"
