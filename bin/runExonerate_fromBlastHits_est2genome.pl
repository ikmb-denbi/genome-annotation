...
while (<MATCHES>){
	...
	system("exonerate --model est2genome --softmaskquery yes --softmasktarget yes --bestn 1 --minintron 20 --maxintron 20000  --showalignment false --showtargetgff true $query.fa $genome_basename/$target.fasta >> $query.exonerate.out");
	system("rm $query.fa");
}
